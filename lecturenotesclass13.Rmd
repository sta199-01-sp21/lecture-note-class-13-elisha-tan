...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-elisha-tan/manhattan.csv")
```
...
- Parameter of interest: The mean price
- Sample: the given sample of 1-bedroom apartments in Manhattan
- Sample size: 20
...
*Identifies response or explanatory variables*
...
*Using your sample to create more samples*
...
*calculate() is from "infer" package; takes argument "stat = [mean, median, sum, prop, count, diff in means, diff in medians, diff in props, Chisq, f, slope, correlation, t, z,  etc]"*
...
*get_ci(dataframe, level = [confidence interval niddle %])
```{r ci_rent_infer}
set.seed(1902370)
...
boot_medians
```
- We can say with 95% confidence that the true median rent price of one-bedroom apartments in Manhattan based on this sample of 20 apartments is between 2,162 and 2,875 dollars.
...
- Population: All mice which were not on treatment
- Parameter of interest: Proportion of mice which died
- Sample: Mice not on treatment used in the experiment
- Sample size: 261 mice
...
set.seed(1902370)
mice_prop <- mice %>% 
  mutate(tr_and_live = treat == 1 & died == 0) %>%
  specify(response = tr_and_live, success = "TRUE") %>% 
  generate(reps = 10000, type = "bootstrap")
mice_prop
...
- We can say with 95% confidence that the true proportion of mice who lived when given any amount of treatment with a sample size of 522 mice is between 0.268 and 0.335. 
